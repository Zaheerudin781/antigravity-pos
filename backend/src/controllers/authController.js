...
exports.register = async (req, res) => {
  try {
    ...
    const restaurant = await Restaurant.create({ tenantId, businessName, currency, currencySymbol, websiteSlug: tenantId });

    // Create admin user
    const user = await User.create({ tenantId, name, email, passwordHash: password, role: 'Admin' });

    // Seed default staff (Admin PIN 1234)
    await Staff.create({ tenantId, name, pin: '1234', role: 'Admin' });

    // Seed default rooms/floors
    const defaultRooms = [
    ...
    // Seed default menu items with costPrices
